...
R.get('/:id', function *(next) {
  ...
  let userGoodCount = yield goodsModel.count({author:article[0].author._id}) //读取用户发布商品总量
  let userArticleCount = yield articlesModel.count({author:article[0].author._id}) //读取用户文章总量
  ...
  if(article.length > 0){
    ...
    if(!this.cookies.get('view')){
      let _article = Object.assign(article[0], {statistics:{view:article[0].statistics.view + 1}});
      yield _article.save();
